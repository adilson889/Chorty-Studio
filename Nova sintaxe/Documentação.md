...
# Listas
ordenar(lista) -> lista
inverter(lista) -> lista
mapear(lista, funcao) -> lista
filtrar(lista, funcao) -> lista

# Sistema
dormir(milisegundos)    # Pausa execução
...
```chor
# Browser (HTML target)
navegador.local_armazenamento
navegador.sessao_armazenamento
navegador.cookies
navegador.geolocalizacao
navegador.notificacoes
...
dispositivo.camera
dispositivo.gps
dispositivo.contatos
dispositivo.notificacoes_push
```